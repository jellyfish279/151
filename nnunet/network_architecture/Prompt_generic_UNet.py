...
from uniseg.nnunet.network_architecture.generic_UNet import ConvDropoutNormNonlin, StackedConvLayers, \
    Upsample, Generic_UNet, StackedConvLayers_multi_channel, Generic_DoDNet
...
class UniSeg_model(Generic_UNet):
    def __init__(self, patch_size, task_total_number, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        super(Generic_UNet, self).__init__()  #uniseg:Generic_UNet; dodnet:Generic__DoDNet
        ...
        final_num_features = final_num_features + 4
    ...
    def forward(self, x):
        ...
        # x = torch.cat(torch.chunk(x, self.modality_num, dim=1), dim=0)#将1,4,96,96,96变为4,1,96,96,96
        ...
        task_prompt = []
        for i in range(4):
            task_prompt.append(torch.select(dynamic_prompt, 1, i))
        task_prompt = torch.stack(task_prompt, dim=1)
...
class DoDNet_model(Generic_DoDNet):
    def __init__(self, patch_size, task_total_number, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 feat_map_mul_on_downscale=2, conv_op=nn.Conv2d,
                 norm_op=nn.BatchNorm2d, norm_op_kwargs=None,
                 dropout_op=nn.Dropout2d, dropout_op_kwargs=None,
                 ...
                 max_num_features=None, basic_block=ConvDropoutNormNonlin,
                 seg_output_use_bias=False):
        super(Generic_DoDNet, self).__init__()  #uniseg:Generic_UNet; dodnet:Generic__DoDNet
        print("training patch size", patch_size)
        self.num_class =  task_total_number
